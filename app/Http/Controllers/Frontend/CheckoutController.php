...
class CheckoutController extends Controller
{
    ...
    public function checkout(){
        if (Auth::check()) {
            if(intval(session('count'))>0){
                ...
                return view('frontend.checkout.customerlogin',compact('data','method_payment','method_ship'));
            }else{
            ...
            if(intval(session('count'))>0){
                ...
                return view('frontend.checkout.guests',compact('data','method_payment','method_ship'));
            }else{
                return redirect()->back();
            }
        }
    ...
    public function postpayment(Request $request){
        ...
        Mail::send('frontend.email.mailordercomlete', array('id' => $max + 1,'name'=>$request->fullname,'email'=>$request->email,'total'=>$request->totalpay), function ($message) use($request) {
            $message->from('contactaobongda@gmail.com', 'Áo bóng đá');
            $message->to($request->email,$request->fullname)->subject('Thông báo đặt hàng thành công');
            $message->to('contactaobongda@gmail.com','Admin')->subject('Thông báo có đơn hàng mới');
        });
        return redirect()->route('frontend.index');
    }
}
